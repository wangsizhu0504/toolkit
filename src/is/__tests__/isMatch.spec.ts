...
describe('isMatch', () => {
  ...
  it(`should match inherited string keyed \`object\` properties`, () => {
    ...
    type FooConstructor = new () => IFoo;

    const Foo = function Foo(this: IFoo) {
      this.a = 1;
  ...
  it(`should work with a non-plain \`object\``, () => {
    ...
    type FooConstructor = new (arg: Partial<IFoo>) => IFoo;

    const Foo = function Foo(this: IFoo, object: Partial<IFoo>) {
  ...
  it(`should return \`true\` when comparing an empty \`source\``, () => {
    const object = { a: 1 };
    const expected = Array.from({ length: 3 }, () => true);
